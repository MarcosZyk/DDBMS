...
def gen_an_article(i):
    ...
    path = '../data/articles/article' + str(i)
    if not os.path.exists(path):
        os.makedirs(path)

    categories = ['business', 'entertainment', 'sport', 'tech']
    random_category = categories[random.randint(0, 3)]
    files = os.listdir('../data/bbc_news_texts/' + random_category + '/')
    size = len(files)
    random_news = files[random.randint(0, size - 1)]
    copyfile('../data/bbc_news_texts/' + random_category + '/' + random_news, path + "/text_a" + str(i) + '.txt')
    ...
    for j in range(image_num):
        copyfile('../data/image/' + str(random.randint(0, 599)) + '.jpg',
                 path + '/image_a' + str(i) + '_' + str(j) + '.jpg')

    # create video
    if random.random() < 0.05:
        # has one video
        article["video"] = "video_a" + str(i) + '_video.mp4'
        if random.random() < 0.5:
            copyfile('../data/video/video1.mp4', path + "/video_a" + str(i) + '_video.mp4')
        else:
            copyfile('../data/video/video2.mp4', path + "/video_a" + str(i) + '_video.mp4')
    else:
        article["video"] = ""
